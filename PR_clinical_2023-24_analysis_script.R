...
flat <- cbind(flat, mediSumRound(dat, "age", 0), # Normal-ish
              ...
              FreqSum(dat, "primary_COPD"),
              FreqSum(dat, "primary_other"),
              ...
              mediSumRound(dat, "ref_to_assess_days", 0),
              mediSumRound(dat, "assess_to_start_days", 0),
              mediSumRound(dat, "ref_to_start_days", 0),
              mediSumRound(dat, "start_to_discharge_days", 0),
              ...
              FreqSum(dat, "EQ5D_init"),
              FreqSum(dat, "EQ5D_mobility_init"),
              FreqSum(dat, "EQ5D_self_care_init"),
              FreqSum(dat, "EQ5D_activities_init"),
              FreqSum(dat, "EQ5D_pain_init"),
              FreqSum(dat, "EQ5D_depression_init"),
              mediSumRound(dat, "EQ5D_utility_index_init", 2),
              mediSumRound(dat, "EQ5D_VAS_init", 0),
              ...
              FreqSum(dat, "MCID_6MWT"), # This one seems easier - look at concordance?
              ...
              FreqSum(dat, "EQ5D_dis"),
              FreqSum(dat, "EQ5D_mobility_diff"),
              FreqSum(dat, "EQ5D_self_care_diff"),
              FreqSum(dat, "EQ5D_activities_diff"),
              FreqSum(dat, "EQ5D_pain_diff"),
              FreqSum(dat, "EQ5D_depression_diff"),
              mediSumRound(dat, "EQ5D_utility_index_diff", 2),
              mediSumRound(dat, "EQ5D_VAS_diff", 0),
              FreqSum(dat, "MCID_EQ5D_utility_index"),
              FreqSum(dat, "MCID_EQ5D_VAS"),
              FreqSum(dat, "MCID_EQ5D_UI_or_VAS"))
...
write.csv(flat.all,
          "G:/Alex Harley/Audit_2023_onwards/2023-2024/PR/Analysis/Output/PR_clinical_2023-24_national_country_org_level_data.csv",
          row.names = FALSE)


# We also need to do the same split by disease now.
...
table(dat$primary_condition)

# we do COPD first and then the others

dat <- dat %>% filter(primary_condition == "COPD")
...
flat <- data.frame(primary_condition = "COPD")
...
flat.all.disease <- flat


# then we do the rest of them
# for this we have to alter dat.save itself

dat.save <- dat.save %>% filter(primary_COPD == "Other disease") %>%
  mutate(primary_condition = fct_drop(primary_condition))


for (i in unique(dat.save$primary_condition)) {
  
  dat <- filter(dat.save, primary_condition == i)
  ...
  flat <- data.frame(primary_condition = i)
  ...
  flat.all.disease <- bind_rows(flat.all.disease, flat)
  
}

str(flat.all.disease)
ncol(flat.all.disease)
nrow(flat.all.disease)
colnames(flat.all.disease)
glimpse(flat.all.disease)


write.csv(flat.all.disease,
          "G:/Alex Harley/Audit_2023_onwards/2023-2024/PR/Analysis/Output/PR_clinical_2023-24_disease_level_data.csv",
          row.names = FALSE)
